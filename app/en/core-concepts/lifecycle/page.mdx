...
> Prerequisites: use factory definitions only, set up NexusRuntime in the root layout, and wrap data‑fetching segments with NexusHydrationBoundary (or export pages with withNexusHydrationBoundary). See /en/getting-started and /en/api/definitions.
...
3) Hydration: collected entries are serialized to the client via NexusHydrationBoundary.
4) Client mounts; NexusRuntime hydrates and restores entries.
