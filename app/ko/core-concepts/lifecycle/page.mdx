...
> 전제: Definition은 팩토리 기반만 사용하고, 루트 레이아웃에 NexusRuntime을 설치하며, 데이터 페칭 세그먼트를 NexusHydrationBoundary로 감쌉니다(또는 페이지를 withNexusHydrationBoundary로 export). 참고: /ko/getting-started, /ko/api/definitions

## 한눈에 보기
1) 팩토리 함수로 엔드포인트 Definition 작성(기본 키는 Definition에서 파생).
2) 서버(RSC)에서 필요한 곳에서 `nexus(...)` 호출 → 응답 헤더/페이로드 수집.
3) 하이드레이션: NexusHydrationBoundary가 수집된 엔트리를 직렬화해 클라이언트로 전달.
4) 클라이언트 마운트 시 NexusRuntime이 복원.
